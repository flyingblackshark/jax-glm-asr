...
# Helper to avoid complex audio_utils dependency if possible, or implement minimal
def make_list_of_audio(audio):
    if isinstance(audio, (list, tuple)):
        return audio
    return [audio]

AudioInput = np.ndarray # Simplified type hint
...
class GlmAsrProcessor(ProcessorMixin):
    ...
    def __init__(
        ...
        self.attributes = {"feature_extractor": "WhisperFeatureExtractor", "tokenizer": "PreTrainedTokenizerFast"}
        # Bypass ProcessorMixin.__init__ dynamic loading which is failing
        # super().__init__(feature_extractor, tokenizer, chat_template=chat_template)
        # We manually set what we need
        self.feature_extractor = feature_extractor
        self.tokenizer = tokenizer
        self.chat_template = chat_template
        # But we need basic mixin functionality?
        # ProcessorMixin.__init__ mainly sets these and handles save_pretrained.
        # If we skip it, save_pretrained might fail or serialization.
        # But for inference it should be fine.
        # Actually calling super().__init__() with no args might work if we set attributes?
        # No, it expects *args matching attributes.
